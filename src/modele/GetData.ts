...
export const ListeAvion = async () => {
    const promise = await fetch("https://webservice-production.up.railway.app/Avions/");
    ...
    for (let index = 0; index < avion.length; index++) {
        ...
        const promiseAssurance = await fetch("https://webservice-production.up.railway.app/Avion/" + element.idAvion + "/Assurances/",{});
...
export const DetailsAvion = async (id: string) => {
    const promise = await fetch("https://webservice-production.up.railway.app/Avion/" + id + "/");
    const avion = await promise.json().then(res => res.data as Avion[]);
    const av=avion[0];
    //kilometrage
    const promiseKilometrage = await fetch("https://webservice-production.up.railway.app/Avion/" + id + "/Kilometrages/", {
        headers: {
    ...
    //entretien
    const promiseDateEntretien = await fetch("https://webservice-production.up.railway.app/Avion/" + id + "/DateEntretiens/", {
        headers: {
            'token': `${localStorage.getItem("token")}`,
            'idPersonne': `${localStorage.getItem("idPersonne")}`
        }
    ...
    //Assurance
    const promiseAssurance = await fetch("https://webservice-production.up.railway.app/Avion/" + id + "/Assurances/");
    const Assurance = await promiseAssurance.json().then(res => res.data as Assurance[]);
